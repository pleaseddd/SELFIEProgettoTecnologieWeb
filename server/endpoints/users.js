...
async function POST_settings(req, res) {
  try {
    ...
    res
      .status(500)
      .json({ error: "errore nel cambiare le impostazioni:" + error });
  }
}

async function POST_setPaletteKey(req, res) {
  try {
    const updatedUser = await userdb.setPaletteKey(
      req.userId,
      req.body.paletteKey
    );
...
async function POST_setLastPomodoro(req, res) {
  try {
    const updatedUser = await userdb.setLastPomodoro(
      req.body.user.userid,
      req.body.user.lastPomodoroSession
    );
